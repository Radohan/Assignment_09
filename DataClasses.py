...
# RadoslawHankiewicz, 2021-Dec-11, Updated the script
#------------------------------------------#
...
class Track():
    ...
    def position(self, value):
        if not int(value).isnumeric():
                raise Exception("Position has to be an integer")
        else:
    ...
    def length(self, value):
        if str(value).isnumeric():
            raise Exception("Length has to be a string")
        else:
            self.__length = value
...
class CD:
    ...
    def rmv_track(self, track_id: int) -> None:
        ...
        del self.__tracks[track_id - 1] # Was not able to figure out how to do it, copied from the Module, but not quite sure how it works.
        self.__sort_tracks()
